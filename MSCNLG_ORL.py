...
class AE_full(object):
    def __init__(self, L1_all, L2_all, sample_num, enc_dim_list, dec_dim_list, num_views=3, para_1=1.0, para_2=1.0,
                 para_3=1.0, learning_rate=1e-3, reg=None, model_path=None, restore_path=None,
                 logs_path='./Net_Models_logs/ORL_logs_multiview'):
    ...
    def partial_fit(self, X, lr):
        ...
        loss, summary, _, coef, lat_rep_1, lat_rep_2, lat_rep_3 = self.sess.run((self.loss, self.merged_summary_op, self.optimizer, self.coef, self.z['0'], self.z['1'], self.z['2']), feed_dict = feed_dict)
        self.summary_weiter.add_summary(summary, self.iter)
        self.iter = self.iter + 1

        return loss, coef, lat_rep_1, lat_rep_2, lat_rep_3
...
def train_AE(x, gt, AE, num_class, sample_num, num_views):
    ...
    for i in range(1):
        ...
        while epoch < max_step:
            epoch = epoch + 1
            cost, coef, lat_rep_1, lat_rep_2,lat_rep_3 = AE.partial_fit(x, lr)
...
def load_data(file_name):
    ...
    x1, x2, x3, gt = dataset['x1_train'], dataset['x2_train'], dataset['x3_train'], dataset['gt_train']
    gt = gt.flatten()
...
if __name__ == '__main__':
    ...
    x0, x1, x2, gt = load_data('F:/dataset/uci.mat')
    ...
    s_tmp = cluster.SpectralClustering(n_clusters=40, n_neighbors=20, eigen_solver="arpack")
    ...
    enc_dim_list = [[6, 256, 128], [240, 256, 128], [47, 256, 128]]
    dec_dim_list = [[256, 6], [256, 240], [256, 47]]

    para_1 = 0.1
    para_2 = 100
    para_3 = 100
    num_class = 10
    model_path = './Net_Models_logs/Model_multiview/ORL_model.ckpt'
    restore_path = './Net_Models_logs/Model_multiview/ORL_model.ckpt'

    tf.reset_default_graph()

    AE = AE_full(L1_all=L1_all, L2_all=L2_all, sample_num=sample_num, enc_dim_list=enc_dim_list,
                 dec_dim_list=dec_dim_list, para_1=para_1, para_2=para_2, para_3=para_3, model_path=None,
                 restore_path=None)
